...
KNOWN_NAMES = {
    "Ben Morton": "Ben",
    "Hanah Smith": "Hanah",
    "Melissa Hawkey": "Melissa",
    "Mohammed Ali": "Mohammed",
    "Rebecca Jackson": "Rebecca",
    "Sara Jones": "Sara",
    "Siobhan Kelly": "Siobhan",
    "Vanessa O’Reilly": "Vanessa"
}
...
def parse_csv_to_monfri(df: pd.DataFrame, week_start: date, known_people: list):
    ...
    if date_col:
        ...
        dbg["date_min"] = "" if dates.dropna().empty else str(dates.dropna().min())[:10]
        dbg["date_max"] = "" if dates.dropna().empty else str(dates.dropna().max())[:10]
        dbg["rows_in_selected_week"] = int(dfw.shape[0])
        if dfw.shape[0] == 0 and not dates.dropna().empty:
            # Suggest the week_start that matches the CSV (nearest Monday to min date)
            try:
                dmin = dates.dropna().min().date()
                suggested = dmin - timedelta(days=dmin.weekday())
                dbg["suggested_week_start"] = suggested.isoformat()
            except Exception:
                pass
            dbg["error"] = "No rows in this CSV match the selected week. Try changing the week selector to match the CSV dates."
...
def apply_extracted_to_week(reports: dict, week_iso: str, metric: str, extracted: dict, known_people: list):
    ...
    # If the week has no people yet, seed it from extracted names (or known people)
    if not block.get("people"):
        block["people"] = sorted(list(extracted_names)) if extracted_names else list(known_people)
    ...
    for d in DAYS:
        for ep, val in extracted.get(d, {}).items():
            tp = name_map.get(ep)
            if not tp and known_people:
                # fallback: map to known people list
                km = build_name_map([ep], known_people, min_score=75)
                tp = km.get(ep)
                if tp and tp not in block["people"]:
                    block["people"].append(tp)
            if not tp:
                continue
...
with tab_uploads:
    ...
    st.caption("No dropdowns 😊 Upload into the right box, hit save, then run the one-click extractor below.")

    t_calls, t_ea, t_td = st.tabs(["📞 Calls", "🏢 EA Calls", "✅ Things Done"])

    def _save_uploads_for_metric(metric_name: str, img_files, csv_files):
        if not img_files and not csv_files:
        ...
        for f in (img_files or []):
            safe_name = f"{w_iso}_{metric_name}_img_{f.name}".replace(" ", "_")
            (P["UPLOADS"] / safe_name).write_bytes(f.getbuffer())
            idx.append({"week_start": w_iso, "metric": metric_name, "filename": safe_name, "kind": "img"})
            saved += 1
        # Save CSVs
        for f in (csv_files or []):
            safe_name = f"{w_iso}_{metric_name}_csv_{f.name}".replace(" ", "_")
            (P["UPLOADS"] / safe_name).write_bytes(f.getbuffer())
            idx.append({"week_start": w_iso, "metric": metric_name, "filename": safe_name, "kind": "csv"})
            saved += 1
        save_index(idx)
        mark_dirty("uploads saved")
        st.success(f"🎉 Saved {saved} file(s) for **{metric_name}**.")
        st.rerun()

    with t_calls:
        metric_name = "Calls"
        up_img_calls = st.file_uploader("📸 Screenshot (PNG/JPG) — Calls", type=["png","jpg","jpeg"], accept_multiple_files=True, key=f"up_img_{TEAM}_{w_iso}_calls")
        up_csv_calls = st.file_uploader("📄 CSV (recommended) — Calls", type=["csv"], accept_multiple_files=True, key=f"up_csv_{TEAM}_{w_iso}_calls")
        if st.button("✅ Save Calls uploads", use_container_width=True, key=f"save_calls_{TEAM}_{w_iso}"):
            _save_uploads_for_metric(metric_name, up_img_calls, up_csv_calls)

    with t_ea:
        metric_name = "EA Calls"
        up_img_ea = st.file_uploader("📸 Screenshot (PNG/JPG) — EA Calls", type=["png","jpg","jpeg"], accept_multiple_files=True, key=f"up_img_{TEAM}_{w_iso}_eacalls")
        up_csv_ea = st.file_uploader("📄 CSV (recommended) — EA Calls", type=["csv"], accept_multiple_files=True, key=f"up_csv_{TEAM}_{w_iso}_eacalls")
        if st.button("✅ Save EA Calls uploads", use_container_width=True, key=f"save_eacalls_{TEAM}_{w_iso}"):
            _save_uploads_for_metric(metric_name, up_img_ea, up_csv_ea)

    with t_td:
        metric_name = "Things Done"
        up_img_td = st.file_uploader("📸 Screenshot (PNG/JPG) — Things Done", type=["png","jpg","jpeg"], accept_multiple_files=True, key=f"up_img_{TEAM}_{w_iso}_thingsdone")
        up_csv_td = st.file_uploader("📄 CSV (recommended) — Things Done", type=["csv"], accept_multiple_files=True, key=f"up_csv_{TEAM}_{w_iso}_thingsdone")
        if st.button("✅ Save Things Done uploads", use_container_width=True, key=f"save_thingsdone_{TEAM}_{w_iso}"):
            _save_uploads_for_metric(metric_name, up_img_td, up_csv_td)
...
if st.button("✨ Extract data and apply to Weekly report", use_container_width=True, key=f"run_all_apply_{TEAM}_{w_iso}"):
    # Show progress message while extraction is happening
    st.info("⏳ Extraction in progress... Please wait. This may take a moment.")
    
    # The existing extraction and application process (OCR/CSV handling)
    people_state = load_people_state()
    ...
    # Show completion message after extraction
    if use_status and status_ctx is not None:
    ...
    st.session_state["last_week_iso"] = w_iso
...
with tab_report:
    ...
    last_iso = st.session_state.get("last_week_iso")
    if last_iso:
        try:
            default_week = date.fromisoformat(last_iso)
        except Exception:
            default_week = monday_of(date.today())
    else:
        default_week = monday_of(date.today())
